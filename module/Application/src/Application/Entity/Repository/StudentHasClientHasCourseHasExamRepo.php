...
class StudentHasClientHasCourseHasExamRepo extends \Doctrine\ORM\EntityRepository
{
	...
	public function getTimingForStudent(StudentHasClientHasCourse $studentInCourse)
	{
        ...
        public function findByStudentCourseExam(StudentHasCourseHasExam $sce) 
        {
            $qb = $this->createQueryBuilder("c")->where("c.studentHasCourseHasExam = :shche");
            $query = $qb->getQuery()->setParameter("shche",$sce);
            $res = $query->getResult();
	...
	{
		$query = $this->getEntityManager()->getConnection()->prepare('SELECT SUM(points) AS tot,student_has_course_id, SUM(TIMESTAMPDIFF(SECOND,real_start_date,end_date)) AS timing FROM student_has_client_has_course_has_exam GROUP BY student_has_has_course_id ORDER BY tot DESC,timing ASC');
		$query->execute();
